...
class PointToPlaneAnalyticFactorTest: public ceres::CostFunction
{
public:

    // Destructor
    ~PointToPlaneAnalyticFactorTest()
    {
        delete finW_opt  ;
        delete n_opt     ;
        delete m_opt     ;
    ...
    // Constructor
    PointToPlaneAnalyticFactorTest(const Vector3d &finW_, const Vector3d &f_, const Vector4d &coef, double w_,
                               int N_, double Dt_, double s_
                               //, ufoSurfelMapPtr &surfMap_, Predicates &commonPred_, ikdtreePtr &ikdtMap_, assocSettings &settings_
                              )
    :   finW       (finW_            ),
        f          (f_               ),
        n          (coef.head<3>()   ),
