...
public:
    ...
    virtual bool Evaluate(double const *const *parameters, double *residuals, double **jacobians) const
    {
        ...
        Mat3 Sftx = SO3d::hat(Xst.S);
        Vec3 OstxPsf = Ostx*Psf;
        Vec3 SftxPsf = Sftx*Psf;
        ...
        Vec3 rA = Xft.A - Xst.A - Xst.R*SftxPsf - Xst.R*(Ostx*OstxPsf);
        ...
        // Jacobians on SO3s states
        {
            ...
            if(jacobians[idx])
            {
                Eigen::Map<Eigen::Matrix<double, 18, 4, Eigen::RowMajor>> Dr_DRsa(jacobians[idx]);
                Dr_DRsa.setZero();
            ...
            if(jacobians[idx])
            {
                Eigen::Map<Eigen::Matrix<double, 18, 3, Eigen::RowMajor>> Dr_DOsa(jacobians[idx]);
                Dr_DOsa.setZero();
            ...
            if(jacobians[idx])
            {
                Eigen::Map<Eigen::Matrix<double, 18, 3, Eigen::RowMajor>> Dr_DSsa(jacobians[idx]);
                Dr_DSsa.setZero();
                ...
                Dr_DSsa.block<3, 3>(9,  0) = DrP_DRst*DXst_DXsa[PIdx][SIdx];
                Dr_DSsa.block<3, 3>(12, 0) = DrV_DRst*DXst_DXsa[VIdx][SIdx] + DrV_DOst*DXst_DXsa[VIdx][SIdx];
            ...
            if(jacobians[idx])
            {
                Eigen::Map<Eigen::Matrix<double, 18, 4, Eigen::RowMajor>> Dr_DRsb(jacobians[idx]);
                Dr_DRsb.setZero();
            ...
            if(jacobians[idx])
            {
                Eigen::Map<Eigen::Matrix<double, 18, 3, Eigen::RowMajor>> Dr_DOsb(jacobians[idx]);
                Dr_DOsb.setZero();
            ...
            if(jacobians[idx])
            {
                Eigen::Map<Eigen::Matrix<double, 18, 3, Eigen::RowMajor>> Dr_DsSb(jacobians[idx]);
                Dr_DsSb.setZero();
        ...
        // Jacobians on SO3f states
        {
            ...
            if(jacobians[idx])
            {
                Eigen::Map<Eigen::Matrix<double, 18, 4, Eigen::RowMajor>> Dr_DRfa(jacobians[idx]);
                Dr_DRfa.setZero();
            ...
            if(jacobians[idx])
            {
                Eigen::Map<Eigen::Matrix<double, 18, 3, Eigen::RowMajor>> Dr_DOfa(jacobians[idx]);
                Dr_DOfa.setZero();
            ...
            if(jacobians[idx])
            {
                Eigen::Map<Eigen::Matrix<double, 18, 3, Eigen::RowMajor>> Dr_DSfa(jacobians[idx]);
                Dr_DSfa.setZero();
            ...
            if(jacobians[idx])
            {
                Eigen::Map<Eigen::Matrix<double, 18, 4, Eigen::RowMajor>> Dr_DRfb(jacobians[idx]);
                Dr_DRfb.setZero();
            ...
            if(jacobians[idx])
            {
                Eigen::Map<Eigen::Matrix<double, 18, 3, Eigen::RowMajor>> Dr_DOfb(jacobians[idx]);
                Dr_DOfb.setZero();
            ...
            if(jacobians[idx])
            {
                Eigen::Map<Eigen::Matrix<double, 18, 3, Eigen::RowMajor>> Dr_DSfb(jacobians[idx]);
                Dr_DSfb.setZero();
        ...
        // Jacobians on PVAs states
        {
            ...
            if(jacobians[idx])
            {
                Eigen::Map<Eigen::Matrix<double, 18, 3, Eigen::RowMajor>> Dr_DPsa(jacobians[idx]);
                Dr_DPsa.setZero();
            ...
            if(jacobians[idx])
            {
                Eigen::Map<Eigen::Matrix<double, 18, 3, Eigen::RowMajor>> Dr_DVsa(jacobians[idx]);
                Dr_DVsa.setZero();
            ...
            if(jacobians[idx])
            {
                Eigen::Map<Eigen::Matrix<double, 18, 3, Eigen::RowMajor>> Dr_DAsa(jacobians[idx]);
                Dr_DAsa.setZero();
            ...
            if(jacobians[idx])
            {
                Eigen::Map<Eigen::Matrix<double, 18, 3, Eigen::RowMajor>> Dr_DRsb(jacobians[idx]);
                Dr_DRsb.setZero();
                Dr_DRsb.block<3, 3>(9,  0) = DrP_DPft*DXst_DXsb[PIdx][PIdx];
                Dr_DRsb.block<3, 3>(12, 0) = DrV_DVft*DXst_DXsb[VIdx][PIdx];
                Dr_DRsb.block<3, 3>(15, 0) = DrA_DAft*DXst_DXsb[AIdx][PIdx];
                Dr_DRsb = sqrtW*Dr_DRsb;
            }
            ...
            if(jacobians[idx])
            {
                Eigen::Map<Eigen::Matrix<double, 18, 3, Eigen::RowMajor>> Dr_DVsb(jacobians[idx]);
                Dr_DVsb.setZero();
                Dr_DVsb.block<3, 3>(9,  0) = DrP_DPft*DXst_DXsb[PIdx][VIdx];
                Dr_DVsb.block<3, 3>(12, 0) = DrV_DVft*DXst_DXsb[VIdx][VIdx];
                Dr_DVsb.block<3, 3>(15, 0) = DrA_DAft*DXst_DXsb[AIdx][VIdx];
            ...
            if(jacobians[idx])
            {
                Eigen::Map<Eigen::Matrix<double, 18, 3, Eigen::RowMajor>> Dr_DAsb(jacobians[idx]);
                Dr_DAsb.setZero();
                Dr_DAsb.block<3, 3>(9,  0) = DrP_DPft*DXst_DXsb[PIdx][AIdx];
                Dr_DAsb.block<3, 3>(12, 0) = DrV_DVft*DXst_DXsb[VIdx][AIdx];
                Dr_DAsb.block<3, 3>(15, 0) = DrA_DAft*DXst_DXsb[AIdx][AIdx];
        ...
        // Jacobians on PVAf states
        {
            ...
            if(jacobians[idx])
            {
                Eigen::Map<Eigen::Matrix<double, 18, 3, Eigen::RowMajor>> Dr_DPfa(jacobians[idx]);
                Dr_DPfa.setZero();
            ...
            if(jacobians[idx])
            {
                Eigen::Map<Eigen::Matrix<double, 18, 3, Eigen::RowMajor>> Dr_DVfa(jacobians[idx]);
                Dr_DVfa.setZero();
            ...
            if(jacobians[idx])
            {
                Eigen::Map<Eigen::Matrix<double, 18, 3, Eigen::RowMajor>> Dr_DAfa(jacobians[idx]);
                Dr_DAfa.setZero();
            ...
            if(jacobians[idx])
            {
                Eigen::Map<Eigen::Matrix<double, 18, 3, Eigen::RowMajor>> Dr_DRfb(jacobians[idx]);
                Dr_DRfb.setZero();
                Dr_DRfb.block<3, 3>(9,  0) = DrP_DPft*DXft_DXfa[PIdx][PIdx];
                Dr_DRfb.block<3, 3>(12, 0) = DrV_DVft*DXft_DXfa[VIdx][PIdx];
                Dr_DRfb.block<3, 3>(15, 0) = DrA_DAft*DXft_DXfa[AIdx][PIdx];
                Dr_DRfb = sqrtW*Dr_DRfb;
            }
            ...
            if(jacobians[idx])
            {
                Eigen::Map<Eigen::Matrix<double, 18, 3, Eigen::RowMajor>> Dr_DVfb(jacobians[idx]);
                Dr_DVfb.setZero();
                Dr_DVfb.block<3, 3>(9,  0) = DrP_DPft*DXft_DXfa[PIdx][VIdx];
                Dr_DVfb.block<3, 3>(12, 0) = DrV_DVft*DXft_DXfa[VIdx][VIdx];
                Dr_DVfb.block<3, 3>(15, 0) = DrA_DAft*DXft_DXfa[AIdx][VIdx];
            ...
            if(jacobians[idx])
            {
                Eigen::Map<Eigen::Matrix<double, 18, 3, Eigen::RowMajor>> Dr_DAfb(jacobians[idx]);
                Dr_DAfb.setZero();
                Dr_DAfb.block<3, 3>(9,  0) = DrP_DPft*DXft_DXfa[PIdx][AIdx];
                Dr_DAfb.block<3, 3>(12, 0) = DrV_DVft*DXft_DXfa[VIdx][AIdx];
                Dr_DAfb.block<3, 3>(15, 0) = DrA_DAft*DXft_DXfa[AIdx][AIdx];
        ...
        // Jacobian of extrinsics
        {
            ...
            if(jacobians[idx])
            {
                Eigen::Map<Eigen::Matrix<double, 18, 4, Eigen::RowMajor>> Dr_DRsf(jacobians[idx]);
                Dr_DRsf.setZero();
            ...
            if(jacobians[idx])
            {
                Eigen::Map<Eigen::Matrix<double, 18, 3, Eigen::RowMajor>> Dr_DPsf(jacobians[idx]);
                Dr_DPsf.setZero();
